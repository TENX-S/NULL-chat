...
static mut MEMBERS: Vec<String> = vec![];
...
async fn connection_loop(mut broker: Sender<Event>, stream: TcpStream) -> Result<()> {
    ...
    unsafe { MEMBERS.push(name.clone()); }
    ...
    while let Some(line) = lines.next().await {
        ...
        let dest: Vec<String> = unsafe { MEMBERS.clone() };
...
async fn connection_writer_loop(messages: &mut Receiver<String>, stream: Arc<TcpStream>, mut shutdown: Receiver<Void>) -> Result<()> {
